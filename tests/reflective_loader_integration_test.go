...
func TestReflectiveLoaderIntegration(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "gomalloader_test_reflective_")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)

	// Write dummy shared library payload (just a NOP sled for test)
	payloadPath := filepath.Join(tempDir, "libdummy.so")
	if err := ioutil.WriteFile(payloadPath, []byte{0x90, 0x90, 0x90, 0x90}, 0755); err != nil {
	...
	if !bytes.Contains(output, []byte("reflective loader error")) {
