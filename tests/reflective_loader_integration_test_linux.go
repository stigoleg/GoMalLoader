...
func TestReflectiveLoaderIntegration_Positive_Linux(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "gomalloader_test_reflective_linux_")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)

	// Build a minimal valid .so shared library
	libSource := `
		#include <stdio.h>
		void test_entry() { printf(\"Hello from test_entry\\n\"); }
	`
	...
	libSoPath := filepath.Join(tempDir, "libdummy.so")
	cmd := exec.Command("gcc", "-shared", "-fPIC", "-o", libSoPath, libCPath)
