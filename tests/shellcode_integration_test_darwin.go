...
func TestShellcodeModeIntegration_Positive_Darwin(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "gomalloader_test_shellcode_darwin_")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)

	// Write shellcode for ret (0xc3) on macOS x86_64
