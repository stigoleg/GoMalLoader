...
func TestReflectiveLoaderIntegration_Positive_Windows(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "gomalloader_test_reflective_windows_")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)

	// Build a minimal valid .dll shared library
	libSource := `
	...
	libDllPath := filepath.Join(tempDir, "libdummy.dll")
	cmd := exec.Command("gcc", "-shared", "-o", libDllPath, libCPath)
	...
	loaderPath := filepath.Join(tempDir, "loader_test_bin.exe")
