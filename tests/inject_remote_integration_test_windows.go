...
func TestInjectRemoteIntegration_Positive_Windows(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "gomalloader_test_inject_windows_")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)

	// Start a dummy process (timeout 10)
	dummyCmd := exec.Command("timeout", "/T", "10")
