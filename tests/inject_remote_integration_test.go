...
func TestInjectRemoteIntegration(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "gomalloader_test_inject_")
	if err != nil {
	...
	// Write dummy shellcode payload (NOP sled)
	...
	if !bytes.Contains(output, []byte("remote injector error")) {
	...
	// Give the dummy process a moment to be affected
